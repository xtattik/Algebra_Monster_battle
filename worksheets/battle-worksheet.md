# Algebra Monster Battle — Battle Worksheet

Show your working for every attack. One attack per block. Print more pages for
more attacks.

Student name: ______________________________

My character: ____________________   My character's stats (M / S / A): ______ / ______ / ______

Environment card: ____________________   Boosts: ____________   Hurts: ____________

My pet: ____________________   Opponent's pet: ____________________

---

One block per attack. Strike attacks: leave the Magic / Agility and environment lines as +0.

```
Attack #1   Attack name & type: ____________________

Base attack expression:            ______________________
+ my Magic / Agility modifier:     ______________________
+ environment modifier:            ______________________
− enemy Strength modifier:         ______________________

Collect like terms  →              ______________________

My dice roll (x) =  ____

Substitute  →                      ______________________  =  ______  damage
(if negative, damage = 0)

Opponent HP:  ______  −  ______  =  ______
```

```
Attack #2   Attack name & type: ____________________

Base attack expression:            ______________________
+ my Magic / Agility modifier:     ______________________
+ environment modifier:            ______________________
− enemy Strength modifier:         ______________________

Collect like terms  →              ______________________

My dice roll (x) =  ____

Substitute  →                      ______________________  =  ______  damage
(if negative, damage = 0)

Opponent HP:  ______  −  ______  =  ______
```

```
Attack #3   Attack name & type: ____________________

Base attack expression:            ______________________
+ my Magic / Agility modifier:     ______________________
+ environment modifier:            ______________________
− enemy Strength modifier:         ______________________

Collect like terms  →              ______________________

My dice roll (x) =  ____

Substitute  →                      ______________________  =  ______  damage
(if negative, damage = 0)

Opponent HP:  ______  −  ______  =  ______
```

---

## Worked example

Sorcerer + Sprite, magic attack, in Arcane Nexus, vs a Barbarian (Average Strength).
Sprite "Fairy Lights" magic attack `3x + 1` · Sorcerer High Magic `+x` · Arcane
Nexus boosts Magic `+x` · enemy Average Strength `0`. Opponent pet starts on 50 HP.
The die roll is 4.

```
Attack #1   Attack name & type: Fairy Lights — magic

Base attack expression:            3x + 1
+ my Magic / Agility modifier:     + x        (High Magic)
+ environment modifier:            + x        (Arcane Nexus boosts Magic)
− enemy Strength modifier:         − 0        (enemy Average Strength)

Collect like terms  →              5x + 1

My dice roll (x) =  4

Substitute  →                      5(4) + 1  =  21  damage
(if negative, damage = 0)

Opponent HP:  50  −  21  =  29
```

---

Show every line even if a modifier is 0 or not used — write `+0`.
